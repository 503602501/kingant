...
//		String imgName = FolderUtil.filterFolderName(url.substring(url.lastIndexOf("/")+1 ) ) ;  //图片名
	    ...
	    imgName = product.getList().get(0)+"_"+product.getHeader().get(product.getDownloadIndex()).replace("图片", "")+".jpg";
//		imgName =ImageUtil.validatePrefix(imgName+"_"+);
		//没有后缀,默认使用png
