...
//					continue;
	...
	/*		//过滤出红色的标签
		...
		/*	webDriver.findElement(By.xpath("//table/thead/tr/th[3]/div/div/span")).click(); 
			...
			//平均排名排序，点击两次
			 ee = webDriver.findElement(By.xpath("//div[contains(text(),'平均展现排名')]/i[1]"));  // 是否需要点击两次？？？？？排序有问题么？
			ee.click();
			Thread.sleep(2000);
			
			ee = webDriver.findElement(By.xpath("//div[@class='adgroup-detail']"));
			 js = String.format("window.scroll(0, %s)", ee.getLocation().getY()+50);  
			 ((JavascriptExecutor)webDriver).executeScript(js);  
